...
pool.connect((err, client, release) => {
    if (err) {
        ...
        // Таблица кэша данных план6 (внутр. plan7) из Oracle — em5.M_360_REPS_PENS_FOR_QLICK_FINAL_CNT
        pool.query(`
            CREATE TABLE IF NOT EXISTS oracle_plan7_cache (
                id INTEGER PRIMARY KEY DEFAULT 1,
                data JSONB NOT NULL DEFAULT '{}',
                fetched_at TIMESTAMP DEFAULT NOW(),
                CHECK (id = 1)
            )
        `).then(() => console.log('✅ Таблица oracle_plan7_cache готова'))
          .catch(e => console.error('❌ oracle_plan7_cache:', e.message));
...
// Маппинг для плана 2: КАТО-код из Oracle → название на сайте
// Вью: cbdiapp.view_es_vipoln_ipr_qlik, SQL выводит "KATO:PROC" (числа, нет проблем с кодировкой)
const ORACLE_KATO_TO_REGION_PLAN2 = {
    ...
    '330000000': 'Область Жетысу',
    '620000000': 'Область Улытау',
    '550000000': 'Павлодарская область',
    '590000000': 'Северо-Казахстанская область',
    '610000000': 'Туркестанская область',
};

// Маппинг для плана 2 (резервный, именной): REGION из Oracle (uppercase) → название на сайте
const ORACLE_REGNAME_TO_REGION_PLAN2 = {
...
// Маппинг для плана 4: числовой ID из Oracle → название на сайте
// Вью: cbdiapp.view_45p_mse_16, SQL выводит "ID:PROC_PRO" (числа, нет проблем с кодировкой)
const ORACLE_ID_TO_REGION_PLAN4 = {
    '1':  'Акмолинская область',
    '2':  'Актюбинская область',
    '3':  'Алматинская область',
    '4':  'Атырауская область',
    '5':  'Восточно-Казахстанская область',  // ВКО
    '6':  'Жамбылская область',
    '7':  'Западно-Казахстанская область',   // ЗКО
    '8':  'Карагандинская область',
    '9':  'Кызылординская область',
    '10': 'Костанайская область',
    '11': 'Мангистауская область',
    '12': 'Павлодарская область',
    '13': 'Северо-Казахстанская область',    // СКО
    ...
    '19': 'Область Жетысу',
    '20': 'Область Улытау',
    // С ведущим нулём (TO_CHAR может вернуть '01'..'09')
...
// Маппинг для плана 6 (внутр. plan7): BRID_NAME из Oracle → название региона на сайте
// ORDER BY BRID: 0100..2000, строка Всего (BRID=9999) исключается в запросе
const ORACLE_BRID_NAME_TO_REGION_PLAN7 = {
    'Акмолинская область':            'Акмолинская область',
    'Актюбинская область':            'Актюбинская область',
    'Алматинская область':            'Алматинская область',
    'Атырауская область':             'Атырауская область',
    ...
    '\u0416\u0435\u0442\u0456\u0441\u0443\u0441\u043a\u0430\u044f \u043e\u0431\u043b\u0430\u0441\u0442\u044c': 'Область Жетысу',  // Жетісуская область
    'Жетісуская область':             'Область Жетысу',
    'Жетисуская область':             'Область Жетысу',
...
app.post('/api/plans/oracle-push', async (req, res) => {
    try {
        ...
        if (oracleRows && Array.isArray(oracleRows)) {
            ...
            } else if (targetPlan === 7) {
                // План 6 (внутр. plan7): элементы содержат regName + cntNaz, cntPlan, procPlan, procFact
                for (const item of oracleRows) {
                    const key = (item.regName || '').trim();
                    const regionName = ORACLE_BRID_NAME_TO_REGION_PLAN7[key];
                    if (!regionName) {
                        console.warn(`⚠️ oracle-push plan7: нет маппинга для "${key}"`);
                        continue;
                    }
                    planSource[regionName] = {
                        kol_del:   parseFloat(item.cntNaz)   || 0,
                        cnt_plan:  parseFloat(item.cntPlan)  || 0,
                        proc_plan: parseFloat(item.procPlan) || 0,
                        proc_fact: parseFloat(item.procFact) || 0,
                    };
                }
            } else if (targetPlan === 4) {
                // План 4: ID-based (regName = числовой ID "01".."20")
                for (const item of oracleRows) {
                    const key = (item.regName || '').trim();
                    const regionName = ORACLE_ID_TO_REGION_PLAN4[key];
                    if (!regionName) {
                        console.warn(`⚠️ oracle-push plan4: нет маппинга для ID "${key}"`);
                        continue;
                    }
                    planSource[regionName] = parseFloat(item.proc) || 0;
                }
            } else if (targetPlan === 2) {
                // План 2: КАТО-based (regName = числовой КАТО-код) или именной как fallback
                for (const item of oracleRows) {
                    const key = (item.regName || '').trim();
                    const regionName = ORACLE_KATO_TO_REGION_PLAN2[key]
                        || ORACLE_REGNAME_TO_REGION_PLAN2[key];
                    if (!regionName) {
                        console.warn(`⚠️ oracle-push plan2: нет маппинга для "${key}"`);
                        continue;
                    }
                    planSource[regionName] = parseFloat(item.proc) || 0;
                }
            } else {
                // Прочие планы: позиционный подход (row → регион)
                for (const item of oracleRows) {
                    console.warn(`⚠️ oracle-push plan${targetPlan}: неизвестный план`);
                }
            }
...
/**
 * GET /api/plans/oracle-plan7
 * Возвращает закэшированные данные плана 6 (внутр. plan7) из Oracle.
 * Данные: { "г. Астана": { kol_del, cnt_plan, proc_plan, proc_fact }, ... }
 */
app.get('/api/plans/oracle-plan7', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT data, fetched_at FROM oracle_plan7_cache WHERE id = 1`
        );
        ...
        console.error('❌ oracle-plan7 ошибка:', error.message);
        res.status(500).json({ error: 'Ошибка при чтении кэша Oracle' });
    }
});
